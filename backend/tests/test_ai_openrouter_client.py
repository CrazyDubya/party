...
class TestOpenRouterClient:
    ...
    async def test_generate_story_success(self, client, mock_response_data):
        ...
        with patch.object(client, "_make_api_request", return_value=mock_response_data):
            result = await client.generate_story(
                premise="Test premise", mood="neutral", characters="2 characters"
            )
    ...
    async def test_generate_story_both_models_fail(self, client):
        ...
        with patch.object(
            client, "_make_api_request", side_effect=Exception("API failed")
        ):
            result = await client.generate_story("Test premise")
    ...
    def test_build_story_prompt(self, client):
        ...
        prompt = client._build_story_prompt(
            premise="Cyberpunk city", mood="dark", characters="detective"
        )
    ...
    def test_parse_story_response_valid_json(self, client, mock_response_data):
        ...
        result = client._parse_story_response(
            mock_response_data, "test premise", "neutral"
        )
    ...
    def test_parse_story_response_invalid_json(self, client):
        ...
        invalid_response = {
            "choices": [
                {"message": {"content": "This is not JSON, just plain text story."}}
            ]
        }

        result = client._parse_story_response(
            invalid_response, "test premise", "neutral"
        )
    ...
    def test_parse_story_response_malformed(self, client):
        ...
        result = client._parse_story_response(
            malformed_response, "test premise", "neutral"
        )
    ...
    def test_count_words(self, client):
        ...
        assert (
            word_count == 15
        )  # Actual word count: "This is chapter one with ten words total" (8) + "This is chapter two with nine words" (7) = 15
    ...
    async def test_test_connection_failure(self, client):
        ...
        with patch.object(
            client, "_make_api_request", side_effect=Exception("Connection failed")
        ):
            result = await client.test_connection()
