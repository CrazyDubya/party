...
class RealAIStoryGenerator:
    ...
    async def generate_story_with_audio(
        self, user_input: str, complexity: str = "medium"
    ) -> Dict[str, Any]:
        ...
        if self.tts_client:
            ...
            for i, chapter in enumerate(story_data["chapters"], 1):
                ...
                if result["success"]:
                    ...
                    audio_results.append(
                        {"chapter": i, "success": False, "error": result["error"]}
                    )
            ...
            "chapters": [
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Attempt direct communication with the artifact using the universal mathematics protocol",
                            "leads_to": 2,
                        },
                        {
                            "id": "b",
                            "text": "Conduct detailed scans to understand the artifact's composition and energy signatures",
                            "leads_to": 3,
                        },
                        {
                            "id": "c",
                            "text": "Establish a quarantine protocol and contact Earth for additional guidance",
                            "leads_to": 4,
                        },
                    ],
                }
            ...
            "chapters": [
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Examine the torn paper more closely and search for the rest of the note",
                            "leads_to": 2,
                        },
                        {
                            "id": "b",
                            "text": "Interview the security guard who discovered the body",
                            "leads_to": 3,
                        },
                        {
                            "id": "c",
                            "text": "Investigate Dr. Voss's recent business dealings and client relationships",
                            "leads_to": 4,
                        },
                    ],
                }
            ...
            "chapters": [
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Ask Pyrathion what price he demands for his help with the barrier",
                            "leads_to": 2,
                        },
                        {
                            "id": "b",
                            "text": "Offer to prove your worthiness through a trial or challenge",
                            "leads_to": 3,
                        },
                        {
                            "id": "c",
                            "text": "Inquire about the nature of the Shadow Plague and who broke the barrier",
                            "leads_to": 4,
                        },
                    ],
                }
    ...
    def _create_adventure_story(
        self, target_words: int, user_input: str
    ) -> Dict[str, Any]:
            ...
            "chapters": [
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Go to Raven's Point lighthouse at midnight as instructed",
                            "leads_to": 2,
                        },
                        {
                            "id": "b",
                            "text": "Research your grandfather's stories and the history of Raven's Point first",
                            "leads_to": 3,
                        },
                        {
                            "id": "c",
                            "text": "Bring a trusted friend for safety despite the 'come alone' instruction",
                            "leads_to": 4,
                        },
                    ],
                }
...
async def main():
    ...
    os.environ[
        "ELEVENLABS_API_KEY"
    ] = "sk_aaf4ec96437deaa159fd672406b4bfbe36b6ac41fcde084d"
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        result = await generator.generate_story_with_audio(
            user_input=test["input"], complexity=test["complexity"]
        )
        ...
        if result["success"]:
            ...
            print(
                f"🔊 Audio Chapters: {result['audio']['chapters_generated']}/{result['audio']['total_chapters']}"
            )
            print(f"💰 Total Cost: ${result['costs']['total']:.6f}")
            print(
                f"⏱️  Generation Time: {result['performance']['generation_time']:.2f}s"
            )
            ...
            if result["audio"]["enabled"]:
                ...
                for audio in result["audio"]["results"]:
                    if audio["success"]:
                        print(
                            f"   Chapter {audio['chapter']}: {audio['audio_size']:,} bytes, ${audio['cost']:.6f}, {audio['voice']} voice"
                        )
                    else:
                        print(
                            f"   Chapter {audio['chapter']}: FAILED - {audio['error']}"
                        )
        else:
