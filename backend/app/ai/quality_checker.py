...
class StoryQualityChecker:
    ...
    def check_story_quality(self, story: Dict) -> QualityResult:
        ...
        quality_score = self._calculate_quality_score(
            issues, word_count, human_likeness
        )
    ...
    def _calculate_human_likeness(self, story: Dict) -> float:
        ...
        sensory_count = sum(
            1 for word in sensory_words if word.lower() in all_text.lower()
        )
        ...
        # AI phrases
        ai_phrase_count = sum(
            1 for phrase in self.ai_indicators if phrase in all_text.lower()
        )
        score -= ai_phrase_count * 5

        # Generic phrases
        generic_count = sum(
            1 for phrase in self.generic_phrases if phrase in all_text.lower()
        )
        score -= generic_count * 3

        # Repetitive structure
        chapter_starts = [
            chapter.get("text", "")[:50] for chapter in story.get("chapters", [])
        ]
        if (
            len(set(chapter_starts)) < len(chapter_starts) * 0.8
        ):  # Less than 80% unique starts
            score -= 10

        return max(0, min(100, score))

    def _calculate_quality_score(
        self, issues: List[Dict], word_count: int, human_likeness: float
    ) -> float:
        ...
        "chapters": [
            {
                ...
                "choices": [
                    ...
                    {
                        "id": "b",
                        "text": "Investigate the strange bird calls",
                        "leads_to": 3,
                    },
                ],
            }
