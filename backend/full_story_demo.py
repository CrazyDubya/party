...
class CompleteStoryDemo:
            ...
            "chapters": [
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Wade into the pond to reach the magical tree",
                            "leads_to": 2,
                        },
                        {
                            "id": "b",
                            "text": "Circle the pond looking for another way across",
                            "leads_to": 3,
                        },
                        {
                            "id": "c",
                            "text": "Call out Tom's name to see if he's nearby",
                            "leads_to": 4,
                        },
                    ],
                },
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Eat another fruit to try entering the mirror world",
                            "leads_to": 5,
                        },
                        {
                            "id": "b",
                            "text": "Take several fruits and return to find help",
                            "leads_to": 6,
                        },
                        {
                            "id": "c",
                            "text": "Touch the tree itself to communicate with it",
                            "leads_to": 7,
                        },
                    ],
                },
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Accept the Guardian's warning and continue",
                            "leads_to": 5,
                        },
                        {
                            "id": "b",
                            "text": "Ask the Guardian to help you understand the danger",
                            "leads_to": 8,
                        },
                        {
                            "id": "c",
                            "text": "Request the Guardian's protection for the journey ahead",
                            "leads_to": 9,
                        },
                    ],
                },
    ...
    async def generate_story_audio(self, story):
        ...
        for i, chapter in enumerate(story["chapters"]):
            ...
            result = await self.tts_client.generate_speech(
                text=chapter["text"], voice=voice, model=TTSModel.FLASH_V2_5
            )
    ...
    def create_playlist_file(self, audio_files, story_title):
        ...
        playlist_path = (
            f"/Users/pup/party/backend/{story_title.replace(' ', '_')}_playlist.m3u"
        )
    ...
    async def run_complete_demo(self):
        ...
        print(
            f"🔊 Total Audio Size: {total_audio_size:,} bytes ({total_audio_size/1024:.1f} KB)"
        )
        ...
        for audio in audio_files:
            print(
                f"   🎵 {audio['title']} ({audio['voice']} voice): {audio['filepath']}"
            )
