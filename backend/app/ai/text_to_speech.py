...
def generate_story_narration(
    story_chapter: Dict, voice_preferences: Optional[Dict] = None
) -> List[Dict]:
    """
    ...
    if "content" in story_chapter:
        ...
        audio_segments.append({"type": "narration", "sequence": 1, **main_audio})
    ...
    if "dialogue" in story_chapter:
        for i, dialogue in enumerate(story_chapter["dialogue"]):
            character_voice = _select_character_voice(
                dialogue.get("character", "unknown")
            )
...
def _select_character_voice(character_name: str) -> VoiceType:
    ...
    if any(word in character_name_lower for word in ["young", "child", "teen"]):
        ...
        word in character_name_lower for word in ["old", "elder", "ancient", "wise"]
    ):
        return VoiceType.CHARACTER_OLD
    elif any(
        word in character_name_lower
        for word in ["shadow", "mysterious", "whisper", "echo"]
    ):
