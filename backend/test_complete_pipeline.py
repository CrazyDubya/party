...
class ElevenLabsWorkingClient:
    ...
    async def generate_speech(
        self, text: str, voice: str = "rachel", model: str = "eleven_flash_v2_5"
    ) -> dict:
...
class CompleteStoryPipeline:
    ...
    async def generate_multimedia_story(
        ...
        print(
            f"✅ Text generated in {text_time:.2f}s ({len(text_result['text'])} characters)"
        )
        ...
        print(
            f"✅ Audio generated in {audio_time:.2f}s ({len(audio_result['audio_data'])} bytes)"
        )
...
async def test_pipeline_performance():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        result = await pipeline.generate_multimedia_story(
            premise=test_case["premise"], voice=test_case["voice"]
        )
        ...
        if result["success"]:
            ...
            results.append(
                {"name": test_case["name"], "success": False, "error": result["error"]}
            )
    ...
    if len(successful_tests) >= 2:
        ...
        print(
            f"✅ Pipeline Success Rate: {len(successful_tests)}/{len(test_cases)} tests"
        )
        print(
            f"✅ Requirement Compliance: {len(passing_tests)}/{len(successful_tests)} tests < 60s"
        )
