...
class TestRealOpenRouterIntegration:
    ...
    async def test_real_story_generation(self, real_client):
        ...
        result = await real_client.generate_story(
            prompt=prompt, model="google/gemini-flash-1.5", max_tokens=500
        )
        ...
        print(
            f"✅ Real Story Generated: {len(result['story'])} characters, Cost: ${result['cost']:.6f}"
        )
    ...
    async def test_real_model_comparison(self, real_client):
        ...
        for model, data in results.items():
            print(
                f"✅ {model}: Cost=${data['cost']:.6f}, Tokens={data['tokens']}, Length={data['quality']}"
            )
...
class TestRealElevenLabsIntegration:
    ...
    async def test_real_speech_generation(self, real_tts_client):
        ...
        result = await real_tts_client.generate_speech(
            text=text, voice="rachel", model=TTSModel.FLASH_V2_5
        )
        ...
        print(
            f"✅ Real Audio Generated: {len(result['audio_data'])} bytes, Cost: ${result['usage'].cost_estimate:.6f}"
        )
    ...
    async def test_real_voice_options(self, real_tts_client):
        ...
        for voice in voices:
            result = await real_tts_client.generate_speech(
                text=text, voice=voice, model=TTSModel.FLASH_V2_5
            )
    ...
    async def test_real_tts_connection(self, real_tts_client):
        ...
        print(
            f"✅ TTS Connection: {result['message']}, Time: {result['generation_time']:.2f}s"
        )
...
class TestRealStableDiffusionIntegration:
    ...
    def real_image_client(self):
        ...
        if not available:
            pytest.skip(
                "No image generation API keys available - skipping real image tests"
            )
    ...
    async def test_real_image_generation(self, real_image_client):
        ...
        result = await real_image_client.generate_image(
            prompt=prompt, size=ImageSize.SQUARE_512
        )

        if result["success"]:
            assert result["image_data"] is not None
            assert (
                len(result["image_data"]) > 5000
            )  # Should have substantial image data
            ...
            print(
                f"✅ Real Image Generated: {len(result['image_data'])} bytes, Provider: {result['provider']}, Cost: ${result['usage'].total_cost:.6f}"
            )
        else:
    ...
    async def test_real_image_connection(self, real_image_client):
        ...
        if result["success"]:
            assert "connection successful" in result["message"]
            print(
                f"✅ Image Connection: {result['message']}, Provider: {result['provider_used']}"
            )
        else:
...
class TestRealStoryGenerationPipeline:
    ...
    async def test_real_complete_story_generation(self, real_story_generator):
        ...
        request = {
            ...
            "include_images": bool(
                os.getenv("RUNWARE_API_KEY") or os.getenv("STABILITY_API_KEY")
            ),
        }
        ...
        total_words = sum(
            len(ch.get("content", "").split()) for ch in result["story"]["chapters"]
        )
        assert total_words >= 100  # Substantial content

        # Check multimedia if APIs available
        if request["include_audio"] and result.get("multimedia", {}).get(
            "audio", {}
        ).get("success"):
            print(
                f"✅ Audio generated: {result['multimedia']['audio']['total_clips']} clips"
            )

        if request["include_images"] and result.get("multimedia", {}).get(
            "image", {}
        ).get("success"):
            print(
                f"✅ Images generated: {len(result['multimedia']['image']['image_data'])} bytes"
            )

        print(
            f"✅ Complete Story: {total_words} words, Cost: ${result['cost']:.6f}, Time: {result['generation_time']:.2f}s"
        )
...
class TestRealQualityAssurance:
    ...
    def test_real_quality_checking(self, quality_checker):
            ...
            "chapters": [
                {
                    ...
                    "choices": [
                        {
                            "id": "a",
                            "text": "Hide behind a large tree and observe who is following",
                            "leads_to": 2,
                        },
                        {
                            "id": "b",
                            "text": "Turn around and confront the person following her",
                            "leads_to": 3,
                        },
                        {
                            "id": "c",
                            "text": "Continue forward quickly to lose whoever is behind her",
                            "leads_to": 4,
                        },
                    ],
                }
        ...
        print(
            f"✅ Quality Check: Score={result.score:.1f}, Human-likeness={result.human_likeness_score:.1f}, Words={result.word_count}"
        )
    ...
    def test_real_cost_tracking(self, cost_optimizer):
        ...
        if model:
            ...
            actual_cost = cost_optimizer.record_request(
                model=model, input_tokens=1000, output_tokens=2000, success=True
            )
            ...
            print(
                f"✅ Cost Tracking: Model={model}, Cost=${actual_cost:.6f}, Budget Used={stats['budget_used_percent']:.1f}%"
            )
...
if __name__ == "__main__":
    ...
    async def quick_test():
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            ...
            if result["success"]:
                print(
                    f"✅ OpenRouter: Generated {len(result['story'])} chars for ${result['cost']:.6f}"
                )
            else:
        ...
        if os.getenv("ELEVENLABS_API_KEY"):
            ...
            if result["success"]:
                print(
                    f"✅ ElevenLabs: Generated {len(result['audio_data'])} bytes for ${result['usage'].cost_estimate:.6f}"
                )
            else:
