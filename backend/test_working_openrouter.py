                ...
                async with session.post(url, headers=headers, json=payload) as response:
                    ...
                    if response.status == 200:
                        ...
                        if "choices" in data and len(data["choices"]) > 0:
                            ...
                            cost = (
                                tokens_used
                                * self.model_costs.get(model, 0.001)
                                / 1000000
                            )
...
async def test_production_deployment():
    ...
    for i, test in enumerate(test_stories, 1):
        ...
        result = await client.generate_text(prompt=test["prompt"], model=test["model"])
    ...
    if success_count >= 1:
        ...
        print(
            f"✅ Story generation working: {success_count}/{len(test_stories)} tests passed"
        )
        ...
        complex_result = await client.generate_text(
            prompt=complex_prompt, model="mistralai/mistral-7b-instruct:free"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Production Status: {'READY' if success else 'NEEDS WORK'} 🚀"
        if success
        else "❌"
    )
