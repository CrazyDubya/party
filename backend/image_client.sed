s/response = await session.post(url, headers=headers, json=payload)/async with session.post(url, headers=headers, json=payload) as response:/g
...
s/async with session.get(image_url) as response:/async with session.get(image_url) as response:/g