...
class StoryGenerator:
    ...
    async def generate_complete_story(
        ...
        try:
            ...
            if "error" in narrative_result:
                return self._create_error_response(
                    "narrative_generation_failed", narrative_result["error"], start_time
                )
            ...
            if not quality_result.valid and progress.elapsed_time < 35:
                ...
                retry_model = (
                    ...
                    if optimal_model == "google/gemini-flash-1.5"
                    else "google/gemini-flash-1.5"
                )

                retry_result = await asyncio.wait_for(
                    self._generate_narrative_specific_model(
                        premise, mood, characters, retry_model
                    ),
                    timeout=20,
                )

                if "error" not in retry_result:
                    retry_quality = self.quality_checker.check_story_quality(
                        retry_result
                    )
            ...
            if include_image and remaining_time > 15:
                multimedia_tasks.append(
                    self._generate_story_image(premise, mood, narrative_result)
                )
            ...
            if multimedia_tasks:
                try:
                    multimedia_timeout = min(
                        remaining_time - 2, 20
                    )  # Reserve 2s for finalization
            ...
            return self._create_error_response("unexpected_error", str(e), start_time)

    async def _generate_narrative_with_fallback(
        self, premise: str, mood: str, characters: str
    ) -> Dict:
        """Generate narrative with automatic fallback"""
        try:
            result = await self.openrouter_client.generate_story(
                premise, mood, characters
            )
    ...
    async def _generate_narrative_specific_model(
        self, premise: str, mood: str, characters: str, model: str
    ) -> Dict:
                        ...
                        audio_clips.append(
                            {
                                ...
                                "duration": audio_result["usage"].characters_used
                                * 0.05,  # Approx 0.05s per char
    ...
    async def _generate_story_image(self, premise: str, mood: str, story: Dict) -> Dict:
        ...
        try:
            ...
            style_prompt = (
                f"{mood} atmosphere, cinematic lighting, detailed illustration"
            )
            context_prompt = (
                f"Based on: {first_chapter[:100]}..." if first_chapter else ""
            )
    ...
    def _create_error_response(
        self, error_type: str, message: str, start_time: float
    ) -> Dict:
    ...
    def get_generation_stats(self) -> Dict:
        ...
        return {
            ...
            "timeout_rate": round(
                (self.timeout_failures / max(self.total_generations, 1)) * 100, 2
            ),
...
async def generate_story(
    premise: str, mood: str = "neutral", characters: str = "3 characters"
) -> Dict:
...
async def example_usage():
    ...
    def progress_callback(progress):
        print(
            f"[{progress.progress_percent}%] {progress.current_task} ({progress.elapsed_time:.1f}s)"
        )
    ...
    if result["success"]:
        ...
        if "multimedia" in result:
            if "audio" in result["multimedia"]:
                print(
                    f"Audio clips: {len(result['multimedia']['audio']['audio_clips'])}"
                )
