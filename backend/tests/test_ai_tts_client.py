...
def client():
    ...
    async def test_generate_speech_success(self, client, mock_audio_response):
        ...
        with patch.object(
            client, "_make_tts_request", return_value=mock_audio_response
        ):
            result = await client.generate_speech(
                text="Hello world!", voice="rachel", model=TTSModel.FLASH_V2_5
            )
    ...
    async def test_generate_speech_api_failure(self, client):
        ...
        with patch.object(
            client, "_make_tts_request", side_effect=Exception("API failed")
        ):
            result = await client.generate_speech("Hello world!")
    ...
    async def test_generate_speech_with_save_path(self, client, mock_audio_response):
        ...
        with patch.object(
            ...
            with patch.object(
                client, "_save_audio_file", return_value=save_path
            ) as mock_save:
                result = await client.generate_speech(
                    text="Hello world!", save_path=save_path
                )
    ...
    @pytest.mark.asyncio
    async def test_generate_speech_custom_voice_settings(
        ...
        with patch.object(
            ...
            result = await client.generate_speech(
                text="Hello world!", voice_settings=custom_settings
            )
    ...
    async def test_test_connection_failure(self, client):
        ...
        with patch.object(
            client, "_make_tts_request", side_effect=Exception("Connection failed")
        ):
            result = await client.test_connection()
...
class TestConvenienceFunctions:
    ...
    async def test_generate_story_audio_fast_mode(self):
        ...
        with patch.object(
            ElevenLabsClient, "generate_speech", return_value=mock_result
        ) as mock_generate:
            result = await generate_story_audio(
                text="Once upon a time...", voice="rachel", fast_mode=True
            )
    ...
    async def test_generate_story_audio_quality_mode(self):
        ...
        with patch.object(
            ...
            result = await generate_story_audio(
                text="Once upon a time...", voice="rachel", fast_mode=False
            )
    ...
    async def test_generate_story_audio_with_save_path(self):
        ...
        with patch.object(
            ...
            result = await generate_story_audio(
                text="Once upon a time...", save_path="/tmp/story.mp3"
            )
...
class TestUsageTracking:
    ...
    def test_usage_calculation_flash_model(self, client_with_usage):
        ...
        char_count = len(text)
        credits_used = int(
            char_count * client_with_usage.model_costs[TTSModel.FLASH_V2_5]
        )
    ...
    def test_usage_calculation_multilingual_model(self, client_with_usage):
        ...
        char_count = len(text)
        credits_used = int(
            char_count * client_with_usage.model_costs[TTSModel.MULTILINGUAL_V2]
        )
        cost_estimate = credits_used * 0.0001
...
class TestErrorHandling:
    ...
    async def test_invalid_model_handling(self):
        ...
        with patch.object(
            client, "_make_tts_request", side_effect=Exception("Invalid model")
        ):
            result = await client.generate_speech(
                text="Hello world!",
    ...
    async def test_file_save_error_handling(self):
        ...
        with patch.object(
            ...
            with patch.object(
                client, "_save_audio_file", side_effect=Exception("Cannot save file")
            ):
                result = await client.generate_speech(
                    text="Hello world!", save_path="/invalid/path/audio.mp3"
                )
...
class TestIntegrationScenarios:
    ...
    async def test_story_generation_pipeline(self):
        ...
        with patch.object(
            ElevenLabsClient, "generate_speech", return_value=expected_result
        ):
            result = await generate_story_audio(
                text=story_text, voice="rachel", fast_mode=True
            )

            assert result["success"] is True
            assert (
                result["usage"].characters_used > 200
            )  # Substantial text (characters_used is int, not list)
    ...
    async def test_multiple_voice_generation(self):
        ...
        for voice in voices_to_test:
            with patch.object(
                client, "_make_tts_request", return_value=mock_audio_data
            ):
                result = await client.generate_speech(
    ...
    async def test_cost_optimization_scenario(self):
        ...
        # Integer credit accounting can differ by one credit after truncation.
        assert abs((flash_credits * 2) - multi_credits) <= 1
