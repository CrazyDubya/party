...
class TestStableDiffusionClient:
    ...
    def mock_runware_response(self):
        ...
        return {
            "data": [
                {"imageURL": "https://cdn.runware.ai/image123.png", "seed": 123456}
            ]
        }
    ...
    def mock_stability_response(self):
        ...
        return {"images": [{"base64": image_b64, "seed": 123456}]}
    ...
    def test_provider_costs_configuration(self, client):
        ...
        assert (
            client.provider_costs[ImageProvider.RUNWARE]
            < client.provider_costs[ImageProvider.STABILITY_AI]
        )
        assert (
            client.provider_costs[ImageProvider.RUNWARE]
            < client.provider_costs[ImageProvider.SEGMIND]
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_image_success_runware(
        self, client, mock_runware_response, mock_image_data
    ):
    ...
    async def test_generate_image_with_save_path(self, client, mock_image_data):
        ...
        with patch.object(
            ...
            with patch.object(
                client, "_save_image_file", return_value=save_path
            ) as mock_save:
                result = await client.generate_image(
                    prompt="Test prompt", save_path=save_path
                )
    ...
    async def test_generate_image_specific_provider(self, client, mock_image_data):
        ...
        with patch.object(
            ...
            result = await client.generate_image(
                prompt="Test prompt", provider=ImageProvider.STABILITY_AI
            )
    ...
    async def test_generate_image_custom_settings(self, client, mock_image_data):
        ...
        custom_settings = ImageSettings(
            steps=50, cfg_scale=10.0, seed=12345, negative_prompt="custom negative"
        )

        with patch.object(
            client,
            "_generate_runware",
            return_value={"success": True, "image_data": mock_image_data},
        ) as mock_generate:
            result = await client.generate_image(
                prompt="Test prompt", settings=custom_settings
            )
    ...
    async def test_generate_image_provider_failure(self, client):
        ...
        with patch.object(
            client, "_generate_runware", side_effect=Exception("API failed")
        ):
            result = await client.generate_image("Test prompt")
    ...
    @pytest.mark.asyncio
    async def test_generate_runware_success(
        self, client, mock_runware_response, mock_image_data
    ):
    ...
    async def test_save_image_file(self, client, mock_image_data):
        ...
        with patch("aiofiles.open", return_value=mock_file):
            with patch("os.makedirs"):
                ...
                mock_file.__aenter__.return_value.write.assert_called_once_with(
                    mock_image_data
                )
    ...
    async def test_test_connection_failure(self, client):
        ...
        with patch.object(
            client, "_generate_runware", side_effect=Exception("Connection failed")
        ):
            result = await client.test_connection()
...
class TestConvenienceFunctions:
    ...
    async def test_generate_story_image_success(self):
        ...
        mock_result = {
            ...
            "image_data": mock_image_data,
            "usage": ImageUsage(
                1, 0.0006, "runware/stable-diffusion-xl", 10.0, "runware"
            ),
        }

        with patch.object(
            StableDiffusionClient, "generate_image", return_value=mock_result
        ) as mock_generate:
            result = await generate_story_image(
                prompt="A magical forest", style="fantasy art"
            )
    ...
    async def test_generate_story_image_with_save_path(self):
        ...
        with patch.object(
            ...
            result = await generate_story_image(
                prompt="A magical forest", save_path="/tmp/story.png"
            )
    ...
    async def test_generate_story_image_optimized_settings(self):
        ...
        with patch.object(
            ...
            assert (
                "text, watermark, signature" in settings.negative_prompt
            )  # Story-specific negatives
    ...
    async def test_invalid_image_size_handling(self):
        ...
        with patch.dict("os.environ", {"RUNWARE_API_KEY": "test_key"}):
            ...
            with patch.object(
                client,
                "_generate_runware",
                return_value={"success": True, "image_data": b"data"},
            ):
                result = await client.generate_image(
                    prompt="Test prompt", size=ImageSize.SQUARE_1024  # Valid size
                )
        ...
        with patch.dict("os.environ", {"RUNWARE_API_KEY": "test_key"}):
            ...
            with patch.object(
                ...
                with patch.object(
                    client,
                    "_save_image_file",
                    side_effect=Exception("Cannot save file"),
                ):
                    result = await client.generate_image(
                        prompt="Test prompt", save_path="/invalid/path/image.png"
                    )
...
class TestIntegrationScenarios:
    ...
    async def test_story_illustration_pipeline(self):
        ...
        with patch.object(
            StableDiffusionClient, "generate_image", return_value=expected_result
        ):
            result = await generate_story_image(
                prompt=story_prompt,
    ...
    async def test_multiple_provider_fallback(self):
        ...
        with patch.dict(
            "os.environ",
            {"RUNWARE_API_KEY": "test_runware", "STABILITY_API_KEY": "test_stability"},
        ):
            client = StableDiffusionClient()
            ...
            with patch.object(
                client, "_generate_with_provider", side_effect=provider_side_effect
            ):
    ...
    async def test_cost_optimization_scenario(self):
        ...
        with patch.dict(
            "os.environ",
            {"RUNWARE_API_KEY": "test_key", "STABILITY_API_KEY": "test_key"},
        ):
            client = StableDiffusionClient()
    ...
    async def test_batch_image_generation(self):
        ...
        with patch.dict("os.environ", {"RUNWARE_API_KEY": "test_key"}):
            ...
            with patch.object(
                ...
                assert (
                    client.total_cost
                    == 4 * client.provider_costs[ImageProvider.RUNWARE]
                )
