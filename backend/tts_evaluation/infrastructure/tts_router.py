...
class TTSRouter:
            ...
            if provider_config.enabled:
                try:
                    client = TTSClientFactory.create_client(
                        provider_config.provider, **provider_config.client_kwargs
                    )
                    ...
                    self.logger.warning(
                        f"Failed to initialize {provider_config.provider.value}: {e}"
                    )
    ...
    def add_provider(self, config: ProviderConfig) -> None:
        ...
        if config.enabled:
            try:
                client = TTSClientFactory.create_client(
                    config.provider, **config.client_kwargs
                )
                ...
                self.logger.error(
                    f"Failed to add provider {config.provider.value}: {e}"
                )
    ...
    async def select_provider(
        self, request: TTSRequest, strategy: RoutingStrategy = RoutingStrategy.BALANCED
    ) -> RoutingDecision:
        """
    ...
    def _get_available_providers(self, request: TTSRequest) -> List[TTSProvider]:
        ...
        for provider, config in self.providers.items():
            ...
            if (
                request.content_type == ContentType.CUSTOM_VOICE
                and not capabilities.supports_voice_cloning
            ):
                continue

            if (
                request.content_type == ContentType.INTERACTIVE
                and capabilities.min_latency > 100
            ):
                continue
    ...
    def _select_cost_optimized(
        self, request: TTSRequest, providers: List[TTSProvider]
    ) -> RoutingDecision:
        ...
        sorted_providers = sorted(
            providers, key=lambda p: self.providers[p].cost_per_char
        )
    ...
    def _select_quality_first(
        ...
        sorted_providers = sorted(
            providers, key=lambda p: self.providers[p].quality_score, reverse=True
        )
    ...
    def _select_latency_optimized(
        ...
        sorted_providers = sorted(
            providers, key=lambda p: self.providers[p].avg_latency
        )
    ...
    def _select_balanced(
        ...
        for provider in providers:
            ...
            cost_score = 1.0 - min(
                config.cost_per_char / 0.0001, 1.0
            )  # Lower cost = higher score
            quality_score = config.quality_score / 10.0
            latency_score = 1.0 - min(
                config.avg_latency / 500.0, 1.0
            )  # Lower latency = higher score

            # Weight based on content type
            if request.content_type == ContentType.BULK:
                composite_score = (
                    0.6 * cost_score + 0.2 * quality_score + 0.2 * latency_score
                )
            elif request.content_type == ContentType.FEATURED:
                composite_score = (
                    0.1 * cost_score + 0.7 * quality_score + 0.2 * latency_score
                )
            elif request.content_type == ContentType.INTERACTIVE:
                composite_score = (
                    0.2 * cost_score + 0.3 * quality_score + 0.5 * latency_score
                )
            else:
                composite_score = (
                    0.4 * cost_score + 0.4 * quality_score + 0.2 * latency_score
                )
    ...
    async def generate_speech(
        ...
        try:
            ...
            # Handle failures with retry
                ...
                self.logger.warning(
                    f"Request failed with {decision.provider.value}: {response.error}"
                )
    ...
    def load_config(self, config_file: str) -> None:
        ...
        try:
            ...
            for provider_name, provider_config in config_data.get(
                "providers", {}
            ).items():
                try:
                    provider = TTSProvider(provider_name)
                    config = ProviderConfig(provider=provider, **provider_config)
                    self.providers[provider] = config
                except ValueError as e:
                    self.logger.warning(
                        f"Invalid provider in config: {provider_name}: {e}"
                    )
