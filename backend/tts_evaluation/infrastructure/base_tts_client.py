...
class BaseTTSClient(ABC):
    ...
    def _validate_request(self, request: TTSRequest) -> None:
        ...
        if len(request.text) > capabilities.max_characters:
            raise ValueError(
                f"Text too long: {len(request.text)} > {capabilities.max_characters}"
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_cost_per_char = (
            (self.total_cost / self.total_characters)
        ...
        return {
            ...
            "characters_per_second": (
                self.total_characters / (self.total_latency / 1000)
            )
...
def estimate_monthly_cost(
    daily_characters: int, cost_per_character: float
) -> Dict[str, float]:
...
def compare_provider_costs(providers_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    sorted_providers = sorted(
        providers_data, key=lambda x: x.get("cost_per_character", float("inf"))
    )
