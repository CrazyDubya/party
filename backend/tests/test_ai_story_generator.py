...
from app.ai.story_generator import (
    ...
    GenerationProgress,
    generate_story,
    test_speed as story_test_speed,
    story_generator as default_story_generator,
)
...
class TestStoryGenerator:
    ...
    async def test_generate_complete_story_success(
        ...
        with patch.object(
            generator, "_determine_complexity", return_value=TaskComplexity.MEDIUM
        ):
            with patch.object(
                generator.cost_optimizer,
                "choose_optimal_model",
                return_value="google/gemini-flash-1.5",
            ):
                with patch.object(
                    generator,
                    "_generate_narrative_with_fallback",
                    return_value=mock_narrative_result,
                ):
                    with patch.object(
                        ...
                        with patch.object(
                            generator,
                            "_generate_audio_clips",
                            return_value=mock_audio_result,
                        ):
                            with patch.object(
                                generator,
                                "_generate_story_image",
    ...
    async def test_generate_complete_story_with_progress_callback(
        ...
        with patch.object(
            generator, "_determine_complexity", return_value=TaskComplexity.SIMPLE
        ):
            with patch.object(
                generator.cost_optimizer,
                "choose_optimal_model",
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        result = await generator.generate_complete_story(
                            premise="Simple story", progress_callback=progress_callback
                        )
                        ...
                        assert any(
                            update["status"] == GenerationStatus.GENERATING_TEXT
                            for update in progress_updates
                        )
                        assert any(
                            update["status"] == GenerationStatus.QUALITY_CHECK
                            for update in progress_updates
                        )
    ...
    async def test_generate_complete_story_budget_exceeded(self, generator):
        ...
        with patch.object(
            generator, "_determine_complexity", return_value=TaskComplexity.HIGH
        ):
            with patch.object(
                generator.cost_optimizer, "choose_optimal_model", return_value=None
            ):
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        generator.quality_checker,
                        "check_story_quality",
                        side_effect=[low_quality, better_quality],
                    ):
                        with patch.object(
                            generator,
                            "_generate_narrative_specific_model",
                            return_value=mock_narrative_result,
                        ):
                            result = await generator.generate_complete_story(
                                "Test premise"
                            ...
                            assert result["success"] is True
                            assert (
                                result["quality"]["score"] == 75
                            )  # Better quality after retry
    ...
    async def test_generate_complete_story_no_multimedia_time_constraint(
        ...
        with patch("time.time") as mock_time:
            ...
            mock_time.side_effect = [0, 0, 0] + [26] * 20
    ...
    async def test_generate_narrative_with_fallback_success(
        ...
        with patch.object(
            ...
            with patch.object(
                generator.cost_optimizer, "record_request"
            ) as mock_record:
    ...
    async def test_generate_audio_clips_success(self, generator, mock_narrative_result):
        ...
        mock_tts_result = {
            ...
            "voice_used": "rachel",
            "generation_time": 1.5,
            "usage": MagicMock(characters_used=100, cost_estimate=0.0001),
        }

        with patch.object(
            generator.tts_client, "generate_speech", return_value=mock_tts_result
        ):
    ...
    @pytest.mark.asyncio
    async def test_generate_audio_clips_failure_fallback(
        self, generator, mock_narrative_result
    ):
        """Test audio generation with failure and fallback"""
        mock_tts_result = {"success": False, "error": "TTS API failed"}
        ...
        with patch.object(
            ...
            assert all(
                "mock-tts-api.com" in clip["audio_url"]
                for clip in result["audio_clips"]
            )

    @pytest.mark.asyncio
    async def test_generate_audio_clips_exception(
        self, generator, mock_narrative_result
    ):
        """Test audio generation exception handling"""
        with patch.object(
            generator.tts_client,
            "generate_speech",
    ...
    async def test_generate_story_image_success(self, generator, mock_narrative_result):
        ...
        with patch.object(
            generator.image_client, "generate_image", return_value=mock_image_result
        ):
            with patch.object(
                generator.image_client,
                "get_usage_stats",
                return_value={"total_images": 1},
            ):
    ...
    @pytest.mark.asyncio
    async def test_generate_story_image_failure_fallback(
        self, generator, mock_narrative_result
    ):
        """Test image generation with failure and fallback"""
        mock_image_result = {"success": False, "error": "Image API failed"}
    ...
    @pytest.mark.asyncio
    async def test_generate_story_image_exception(
        self, generator, mock_narrative_result
    ):
    ...
    def test_determine_complexity_simple(self, generator):
        ...
        complexity = generator._determine_complexity("Short story", "happy", "2 people")
    ...
    def test_create_error_response(self, generator):
        ...
        response = generator._create_error_response(
            "test_error", "Something went wrong", start_time
        )
    ...
    def test_create_error_response_budget_exceeded(self, generator):
        ...
        response = generator._create_error_response(
            "budget_exceeded", "No budget", time.time()
        )
    ...
    def test_get_generation_stats(self, generator):
        ...
        with patch.object(
            generator.cost_optimizer, "get_daily_stats", return_value={"cost": 0.05}
        ):
    ...
    @pytest.mark.asyncio
    async def test_test_generation_speed(
        self, generator, mock_narrative_result, mock_quality_result
    ):
        ...
        with patch.object(
            ...
            return_value={"success": True, "quality": {"word_count": 150, "score": 85}},
        ):
...
class TestConvenienceFunctions:
    ...
    async def test_generate_story_convenience(self):
        ...
        with patch.object(
            default_story_generator, "generate_complete_story", return_value=mock_result
        ):
    ...
    async def test_test_speed_convenience(self):
        ...
        with patch.object(
            default_story_generator, "test_generation_speed", return_value=mock_result
        ):
            result = await story_test_speed()
...
async def test_speed():
    ...
    mock_result = {"test_completed": True, "generation_time": 5.5, "within_limit": True}
...
class TestTimeoutScenarios:
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    generator,
                    "_generate_narrative_with_fallback",
                    side_effect=slow_text_generation,
                ):
    ...
    async def test_timeout_during_multimedia_generation(self):
        ...
        with patch.object(
            generator.cost_optimizer, "choose_optimal_model", return_value="test-model"
        ):
            with patch.object(
                generator,
                "_generate_narrative_with_fallback",
                ...
                with patch.object(
                    ...
                    with patch.object(
                        generator, "_generate_audio_clips", side_effect=slow_multimedia
                    ):
                        with patch.object(
                            generator,
                            "_generate_story_image",
                            side_effect=slow_multimedia,
                        ):
                            result = await generator.generate_complete_story(
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        generator, "_generate_audio_clips", return_value=mock_audio
                    ):
                        with patch.object(
                            generator,
                            "_generate_story_image",
                            side_effect=mock_image_error,
                        ):
                            result = await generator.generate_complete_story("Test")
...
class TestIntegrationScenarios:
    ...
    async def test_full_pipeline_with_all_features(self):
        ...
        mock_audio = {
            "success": True,
            "audio_clips": [
                {"chapter_id": i, "duration": 5.0, "cost": 0.0002} for i in range(1, 4)
            ],
            "total_clips": 3,
            "total_duration": 15.0,
            "provider": "elevenlabs",
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    with patch.object(
                        ...
                        with patch.object(
                            generator, "_generate_story_image", return_value=mock_image
                        ):
                ...
                with patch.object(
                    ...
                    tasks = [
                        generator.generate_complete_story(
                            f"Story {i}", include_audio=False, include_image=False
                        )
                        for i in range(5)
                    ]
