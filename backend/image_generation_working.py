...
class WorkingImageClient:
    ...
    async def generate_image(
        ...
        try:
            if provider == ImageProvider.RUNWARE:
                ...
                return ImageGenerationResult(
                    success=False, error=f"Provider {provider.value} not implemented"
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    self.providers[ImageProvider.RUNWARE]["url"],
                    headers=headers,
                    json=payload,
                ) as response:
                    if response.status == 200:
                        ...
                        if data.get("data") and len(data["data"]) > 0:
                            ...
                            return {
                                "success": False,
                                "error": "No image data in Runware response",
                            }
                    else:
                        error_text = await response.text()
                        return {
                            "success": False,
                            "error": f"Runware API error: {response.status} - {error_text}",
                        }
    ...
    async def _generate_segmind(self, prompt: str, size: ImageSize) -> Dict:
        ...
        headers = {"x-api-key": self.segmind_key, "Content-Type": "application/json"}
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    self.providers[ImageProvider.SEGMIND]["url"],
                    headers=headers,
                    json=payload,
                ) as response:
                    if response.status == 200:
                        ...
                        if data.get("image"):
                            ...
                            return {
                                "success": False,
                                "error": "No image data in Segmind response",
                            }
                    else:
                        error_text = await response.text()
                        return {
                            "success": False,
                            "error": f"Segmind API error: {response.status} - {error_text}",
                        }
    ...
    async def _generate_stability(self, prompt: str, size: ImageSize) -> Dict:
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    self.providers[ImageProvider.STABILITY_AI]["url"],
                    headers=headers,
                    json=payload,
                ) as response:
                    if response.status == 200:
                        ...
                        if data.get("images") and len(data["images"]) > 0:
                            ...
                            return {
                                "success": False,
                                "error": "No image data in Stability AI response",
                            }
                    else:
                        error_text = await response.text()
                        return {
    ...
    def get_usage_stats(self) -> Dict:
        ...
        return {
            ...
            "average_cost_per_image": round(
                self.total_cost / max(self.total_images, 1), 4
            ),
            "providers_available": [
                p.value for p, config in self.providers.items() if config["available"]
            ],
            "cheapest_provider": "runware"
            if self.providers[ImageProvider.RUNWARE]["available"]
            else "segmind",
        }
...
async def generate_story_image(
    prompt: str, style: str = "digital art, cinematic", save_path: Optional[str] = None
) -> ImageGenerationResult:
...
async def test_image_generation():
    ...
    for i, prompt in enumerate(test_prompts, 1):
        ...
        result = await client.generate_image(
            prompt=prompt, save_path=f"/Users/pup/party/test_image_{i}.png"
        )
