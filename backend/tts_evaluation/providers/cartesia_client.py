...
class CartesiaClient(BaseTTSClient):
        ...
        try:
            ...
            payload = {
                ...
                "emotion": self._map_emotion(
                    request.voice_settings.emotion
                    if request.voice_settings
                    else "neutral"
                ),
            }
    ...
    def _map_voice(self, voice_preference: str) -> str:
        ...
        return voice_mapping.get(
            voice_preference.lower(), self.default_voices["neutral"]
        )

    def _map_output_format(self, format_name: str) -> str:
        """Map output format to Cartesia format"""
        format_mapping = {"mp3": "mp3", "wav": "wav", "flac": "flac", "ogg": "ogg"}
    ...
    async def create_voice_clone(
        ...
        try:
            ...
            data.add_field(
                "audio", audio_data, filename=os.path.basename(audio_file_path)
            )
            ...
            return {"success": False, "error": f"Voice cloning error: {str(e)}"}
            ...
            async with websockets.connect(
                ...
                async for message in websocket:
                    ...
                    if data.get("type") == "chunk":
                        ...
                        if audio_chunk:
                            yield audio_chunk.encode() if isinstance(
                                audio_chunk, str
                            ) else audio_chunk
    ...
    def estimate_cost(
        self, character_count: int, quality_level: QualityLevel = QualityLevel.STANDARD
    ) -> float:
...
async def cartesia_voice_clone(
    audio_file: str, voice_name: str, description: str = ""
) -> Dict[str, Any]:
    """
...
async def benchmark_cartesia_speed(test_texts: List[str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "average_latency": sum(r.get("latency", 0) for r in results if r["success"])
