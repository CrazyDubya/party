...
async def demo_real_tts():
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        result = await client.generate_speech(
            text=test["text"], voice=test["voice"], model=TTSModel.FLASH_V2_5
        )
    ...
    print(f"\n📊 TOTALS:")
    print(
        f"   🔊 Total Audio: {total_audio_size:,} bytes ({total_audio_size/1024:.1f} KB)"
    )
    print(f"   💵 Total Cost: ${total_cost:.6f}")
    print(
        f"   📈 Average Cost per Character: ${total_cost/sum(len(t['text']) for t in test_cases):.8f}"
    )
                ...
                "chapters": [
                    {
                        ...
                        "choices": [{"id": "a", "text": "Go", "leads_to": 2}],
                    }
                ],
            },
        },
...
def demo_real_cost_optimization():
    ...
    print(
        f"   🎯 Budget Status: {'Over Budget!' if stats['is_over_budget'] else 'Within Budget'}"
    )
...
def create_real_ai_config():
    ...
    print(
        f"🔊 ElevenLabs: {'✅ ENABLED' if config['apis']['elevenlabs']['enabled'] else '❌ DISABLED'}"
    )
    print(
        f"🧠 OpenRouter: {'✅ ENABLED' if config['apis']['openrouter']['enabled'] else '❌ DISABLED'}"
    )
    print(
        f"🎨 Image Gen: {'✅ ENABLED' if config['apis']['image_generation']['enabled'] else '❌ DISABLED'}"
    )
