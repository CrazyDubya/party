...
class TestCostOptimizer:
    ...
    def test_estimate_request_cost_known_model(self, optimizer):
        ...
        cost = optimizer.estimate_request_cost(
            "google/gemini-flash-1.5", input_tokens=1000, estimated_output_tokens=2000
        )
    ...
    def test_estimate_request_cost_unknown_model(self, optimizer):
        ...
        cost = optimizer.estimate_request_cost(
            "unknown/model", input_tokens=1000, estimated_output_tokens=1000
        )
    ...
    def test_get_daily_stats_current_day(self, optimizer):
        ...
        with patch.object(
            optimizer, "_get_daily_spend", return_value=15.5
        ) as mock_spend:
            with patch.object(
                optimizer,
                "_get_daily_requests",
    ...
    def test_get_daily_spend(self, optimizer):
        ...
        optimizer.cost_history = [
            ...
            CostEntry(
                test_timestamp - 3600, "model3", 150, 250, 0.006, "task3", True
            ),  # Previous day
            CostEntry(
                test_timestamp + 90000, "model4", 300, 400, 0.012, "task4", True
            ),  # Next day
        ]
    ...
    def test_get_daily_requests(self, optimizer):
        ...
        optimizer.cost_history = [
            ...
            CostEntry(
                test_timestamp - 1000, "model4", 300, 400, 0.012, "task4", True
            ),  # Previous day
        ]
    ...
    def test_save_cost_history_limits_entries(self, optimizer, temp_cost_file):
        ...
        assert (
            data["cost_history"][0]["timestamp"] == 500
        )  # Should start from entry 500
...
class TestConvenienceFunctions:
    ...
    def test_get_optimal_model_simple(self):
        ...
        with patch(
            "app.ai.cost_optimizer.cost_optimizer.choose_optimal_model",
    ...
    def test_get_optimal_model_high(self):
        ...
        with patch(
            "app.ai.cost_optimizer.cost_optimizer.choose_optimal_model",
            return_value="anthropic/claude-3-sonnet",
        ) as mock_choose:
    ...
    def test_can_afford_function(self):
        ...
        with patch(
            "app.ai.cost_optimizer.cost_optimizer.estimate_request_cost",
            return_value=0.005,
        ) as mock_estimate:
            with patch(
                "app.ai.cost_optimizer.cost_optimizer.can_afford_request",
                return_value=(True, {"budget": "details"}),
            ) as mock_afford:
    ...
    def test_record_usage_function(self):
        ...
        with patch(
            "app.ai.cost_optimizer.cost_optimizer.record_request", return_value=0.008
        ) as mock_record:
            cost = record_usage("anthropic/claude-3-haiku", 800, 1200, True)

            assert cost == 0.008
            mock_record.assert_called_once_with(
                "anthropic/claude-3-haiku", 800, 1200, success=True
            )

    def test_record_usage_function_with_failure(self):
        """Test record usage convenience function with failure"""
        with patch(
            "app.ai.cost_optimizer.cost_optimizer.record_request", return_value=0.012
        ) as mock_record:
            cost = record_usage("anthropic/claude-3-sonnet", 1000, 800, False)

            assert cost == 0.012
            mock_record.assert_called_once_with(
                "anthropic/claude-3-sonnet", 1000, 800, success=False
            )
...
class TestBudgetManagement:
    ...
    def test_daily_budget_reset(self, budget_optimizer):
        ...
        budget_optimizer.cost_history = [
            CostEntry(
                yesterday_timestamp + 3600, "model", 1000, 2000, 20.0, "task", True
            )
        ]
...
class TestCostCalculationAccuracy:
    ...
    def test_gemini_flash_cost_precision(self, precise_optimizer):
        ...
        for input_tokens, output_tokens, expected in test_cases:
            cost = precise_optimizer.estimate_request_cost(
                "google/gemini-flash-1.5", input_tokens, output_tokens
            )
            assert cost == pytest.approx(expected, abs=1e-6)
    ...
    def test_claude_haiku_cost_precision(self, precise_optimizer):
        ...
        for input_tokens, output_tokens, expected in test_cases:
            cost = precise_optimizer.estimate_request_cost(
                "anthropic/claude-3-haiku", input_tokens, output_tokens
            )
            assert cost == pytest.approx(expected, abs=1e-6)
    ...
    def test_claude_sonnet_cost_precision(self, precise_optimizer):
        ...
        for input_tokens, output_tokens, expected in test_cases:
            cost = precise_optimizer.estimate_request_cost(
                "anthropic/claude-3-sonnet", input_tokens, output_tokens
            )
            assert cost == pytest.approx(expected, abs=1e-6)
...
class TestIntegrationScenarios:
    ...
    def test_typical_workday_scenario(self, production_optimizer):
        ...
        for complexity, input_tokens, output_tokens in story_requests:
            ...
            if model:
                cost = production_optimizer.estimate_request_cost(
                    model, input_tokens, output_tokens
                )
                total_estimated_cost += cost

                # Record the request
                production_optimizer.record_request(
                    model, input_tokens, output_tokens, success=True
                )
    ...
    def test_error_recovery_scenario(self, production_optimizer):
        ...
        for model, input_tokens, output_tokens, success in requests:
            production_optimizer.record_request(
                model, input_tokens, output_tokens, success=success
            )
