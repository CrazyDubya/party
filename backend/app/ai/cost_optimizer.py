...
class CostOptimizer:
    ...
    def estimate_request_cost(
        self, model: str, input_tokens: int, estimated_output_tokens: int = 1000
    ) -> float:
    ...
    def choose_optimal_model(
        self, complexity: TaskComplexity, budget_conscious: bool = True
    ) -> str:
    ...
    def _save_cost_history(self):
        ...
        try:
            data = {
                "cost_history": [
                    asdict(entry) for entry in self.cost_history[-1000:]
...
def record_usage(
    ...
    """Record API usage"""
    return cost_optimizer.record_request(
...
if __name__ == "__main__":
    ...
    print(
        "Optimal model for simple task:",
        optimizer.choose_optimal_model(TaskComplexity.SIMPLE),
    )
    print(
        "Optimal model for complex task:",
        optimizer.choose_optimal_model(TaskComplexity.HIGH),
    )
